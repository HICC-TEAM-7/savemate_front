...
const SpendingList: React.FC<Props> = ({ from, to, title = "소비 내역", className }) => {
  const [loading, setLoading] = useState(false);
  const [days, setDays] = useState<DaySpending[]>([]);

  // 기본 범위: 이번 달 전체
  const fallbackRange = useMemo(() => getMonthRange(new Date(), { upToToday: false }), []);
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className="py-6 text-center text-gray-300">표시할 내역이 없어요</div>
      ) : (
        <div>
